 ...
 * anders neerzetten, gebruik dan het filter 'cf_campagne_update_manifest'.
 */
...
class CF_Campagne_Updater {

	/** Waar de laatste versie staat. */
	const MANIFEST = 'https://raw.githubusercontent.com/marketing-chiro/pop-up/claude/chiro-fysio-exit-popup-eahq6b/dist/update-campagnes.json';

	/** Hoe lang we het antwoord bewaren, zodat we niet bij elk beheerbezoek kijken. */
	const CACHE_KEY = 'cf_campagne_update_check';
	...
	private static function bestandsnaam() {
		return plugin_basename( CF_CAMPAGNE_FILE );
	}
	...
	private static function op_afstand() {
		 ...
		 */
		$url = apply_filters( 'cf_campagne_update_manifest', self::MANIFEST );
	...
	private static function adres_vertrouwd( $download_url ) {
		$bron = apply_filters( 'cf_campagne_update_manifest', self::MANIFEST );
	...
	public static function aanbieden( $transient ) {
		...
		if ( version_compare( $data['version'], CF_CAMPAGNE_VERSION, '>' ) ) {
	...
	public static function details( $resultaat, $actie, $args ) {
		...
		return (object) array(
			'name'          => isset( $data['name'] ) ? $data['name'] : 'Chiro-Fysio e-mailcampagnes',
			'slug'          => self::slug(),
